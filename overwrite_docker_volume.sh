...
echo "You must stop related containers before proceed"
read -p "Press enter to continue or Ctrl+C to break script"
...
function validateInput() {
    ...
    if [[ ${INSPECT_VOLUME} == *"No such volume"* ]] ; then
        echo "> Error: docker volume '${DOCKER_RESTORE_VOLUME}' does not exist "
        exit 1
    fi
}
...
${DOCKER_IMAGE} \
sh -c "cd /backup-dest && rm -rfv * && rm -rfv .*"
...
echo "> Finished! Docker volume '${DOCKER_RESTORE_VOLUME}' is overwritten with backup file contents"
